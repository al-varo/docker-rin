...
sql_in = "SELECT sm.name, product_uos_qty, (select name from product_uom where id=sm.product_uom) from stock_move sm where date >= '{}' and origin like 'PO%' and sm.name not like 'ROKOK %' and location_dest_id=12 and state='done' order by sm.name asc"
sql_draft = "SELECT \
        ...
        if len(command) > 1:
            try:
                ...
                for r in command:
                    if r != "/lapor":
                        x=x+r+" "
                z="Laporan atau request telah dikirim."
